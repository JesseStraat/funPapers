...
\begin{multicols}{2}
    ...
    The question remains whether there exists a physical alternative to Hilbert spaces. One popular alternative to Hilbert spaces is the rigged Hilbert space\cite{de_la_madrid_role_2005}. These are Hilbert spaces together with a certain restriction. However, rigged Hilbert spaces keep the original Hilbert spaces, so the same issues remain.\\
    \begin{figure}[H]
        \centering
        \resizebox{\columnwidth}{!}{
        \input{statesketch.tex}
        }
        \caption{A one-dimensional example of a quantum wave whose position expectation value diverges to infinity. The peaks exist at all powers of \(2\) with height proportional to \(2^{-x/2}\).}
        \label{fig:sketch}
    \end{figure}\noindent
    The authors do leave us with another possible alternative called Schwartz spaces\cite{becnel_schwartz_2015}, which lose the original Hilbert space, keeping only the non-problematic states. Indeed, they are formed exactly by removing all states whose (combinations of) momentum and position expectations diverge, thus losing completeness. However, no conclusion is drawn on the physicality of Schwartz spaces, and further research should be conducted to investigate the possibility.
\end{multicols}
Word count: 625
\printbibliography
\end{document}