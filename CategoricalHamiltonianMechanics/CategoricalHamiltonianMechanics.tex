...
\begin{multicols}{2}
	...
	\begin{proof}
		...
		\begin{align*}% TODO: check if F(f) should be f for proto-Hamiltonian theories
			G(f)^*&G(\eta,\,K)(\phi) \\
			&= \omega(F(\eta,\,K)(\phi),\,F(\eta,\,K)(H))\circ G(f)\\
			&= \omega(G(f)^*F(\eta,\,K)(\phi),\,G(f)^*F(\eta,\,K)(H))\\
			&= \omega(F(\omega,\,H)(G(f)^*\phi),\,F(\omega,\,H)(G(f)^*H))\\
			&= G(\omega,\,H)(G(f)^*\phi).
		\end{align*}
